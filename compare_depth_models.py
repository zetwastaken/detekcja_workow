        ...
        if run_folder.is_dir() and run_folder.name.startswith("depth_"):
            ...
            if results_file.exists():
                ...
                if len(parts) >= 3:
                    ...
                    model_name = "_".join(parts[1:-2])  # Remove 'depth_' prefix and timestamp
...
def print_ranking(df: pd.DataFrame):
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
        ...
        print(f"{medal} {model_type_icon} {row['Model']:25} | "
    ...
    if not depth_models.empty and not rgb_models.empty:
        ...
        improvement = ((best_depth['mAP50-95'] - rgb_baseline['mAP50-95']) / rgb_baseline['mAP50-95']) * 100
        
        print(f"\n📈 DEPTH vs RGB COMPARISON:")
        print(f"   Best Depth Model: {best_depth['Model']} (mAP50-95: {best_depth['mAP50-95']:.4f})")
        print(f"   RGB Baseline:     {rgb_baseline['Model']} (mAP50-95: {rgb_baseline['mAP50-95']:.4f})")
...
def plot_comparison(df: pd.DataFrame, results: list, output_dir: Path):
    ...
    bars = ax.barh(df['Model'], df['mAP50-95'], color=colors, edgecolor='white', linewidth=0.5)
    ...
    for r in results:
        if 'results_df' in r:
            ...
            if map_col:
                ax.plot(df_train['epoch'], df_train[map_col[0]], 
                       label=r['model_name'], linewidth=1.5, alpha=0.8)
    
    ax.set_xlabel('Epoch', fontsize=12)
    ax.set_ylabel('mAP50-95', fontsize=12)
    ax.set_title('Training Curves - mAP50-95 Over Epochs', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-rgb",
        action="store_true",
        help="Exclude RGB baseline from comparison"
    )
    parser.add_argument(
        "--no-export",
        action="store_true",
