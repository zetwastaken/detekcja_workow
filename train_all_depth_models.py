...
def train_single_dataset(model_name: str, data_yaml: Path, 
                         base_model: str = "yolov8n-seg.pt") -> dict:
    """
...
def main(datasets_to_train: list = None, base_model: str = "yolov8n-seg.pt"):
    ...
    if not all_datasets:
        ...
        print(f"Run generate_depth_datasets.py first to create datasets in {DATASETS_DIR}")
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--include-rgb",
        action="store_true",
        help="Also train on RGB baseline dataset"
    )
